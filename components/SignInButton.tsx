...
export default function SignInButton({}: Props) {
  ...
  if (!address) {
    return <ConnectWallet className="hover:bg-purple-600 border-purple-600 border bg-purple-500 text-white px-3 py-2 text-sm focus:ring-0" />;
  }

  // 2. User needs to switch network to Polygon
  if (isOnWrongNetwork) {
    return (
      <button onClick={() => switchNetwork?.(ChainId.Polygon)} className="hover:bg-purple-600 border-purple-600 border bg-purple-500 text-white px-3 py-2 text-sm focus:ring-0">
        Switch Network
      </button>
    );
  }
  ...
  if (isSignedInQuery.isLoading) {
    return <div className="hover:bg-purple-600 border-purple-600 border bg-purple-500 text-white px-3 py-2 text-sm focus:ring-0" >Loading...</div>;
  }
  ...
  if (!profileQuery.data?.defaultProfile) {
    return <div className="hover:bg-purple-600 border-purple-600 border bg-purple-500 text-white px-3 py-2 text-sm focus:ring-0" >No Lens Profile.</div>;
  }
  ...
  return <div className="hover:bg-purple-600 border-purple-600 border bg-purple-500 text-white px-3 py-2 text-sm focus:ring-0" >Something went wrong.</div>;
}
